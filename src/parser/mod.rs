...
pub fn parse_with_recovery(source: &str) -> (Program, Vec<MireError>) {
    parse_with_recovery_known(source, &HashSet::new(), &HashSet::new(), &HashMap::new())
}

/// Like [`parse_with_recovery`] but seeds the parser's nominal type / enum name
/// tables with names known from an enclosing program. This lets a fragment that
/// only contains an `impl` (generated by derive, etc.) still resolve references
/// to struct/enum types declared elsewhere (e.g. `impl Point` referring to
/// `other :&Point`), which `collect_top_level_metadata` alone cannot see.
pub fn parse_with_recovery_known(
    source: &str,
    extra_nominal: &HashSet<String>,
    extra_enum: &HashSet<String>,
    extra_owners: &HashMap<String, String>,
...
impl Parser {
    ...
    /// Like [`Parser::new`] but merges extra nominal/enum type names known from
    /// an enclosing program into the parser's lookaside tables.
    pub fn new_with_known_types(
        tokens: Vec<Token>,
        extra_nominal: &HashSet<String>,
        extra_enum: &HashSet<String>,
        extra_owners: &HashMap<String, String>,
    ) -> Self {
        let (mut enum_names, mut enum_variant_owners, mut nominal_type_names) =
            Self::collect_top_level_metadata(&tokens);
        for name in extra_nominal {
            nominal_type_names.insert(name.clone());
        }
        for name in extra_enum {
